...
| Тип | Назва | Опис |
|-----|-------|------|
| Agent | [task-refiner](../agents/task-refiner.md) | Оркестратор уточнення задач — координує researchers, аналізує, діалог з PM (Opus) |
| Agent | [jira-researcher](../agents/jira-researcher.md) | Sub-agent: збір контексту з Jira (Sonnet) |
| Agent | [confluence-researcher](../agents/confluence-researcher.md) | Sub-agent: пошук документації в Confluence (Sonnet) |
| Agent | [sentry-researcher](../agents/sentry-researcher.md) | Sub-agent: перевірка production health (Sonnet) |
| Agent | [codebase-scanner](../agents/codebase-scanner.md) | Sub-agent: сканування кодової бази — local або GitHub (Sonnet) |
...
Зберігаються в `~/.claude/settings.json` → секція `env` (Claude Code гарантовано читає при запуску):
...
Запустіть `/pm:setup` для інтерактивного налаштування.
